<script lang="ts">
  ...
  // Stable marker class for test queries
  const selectionMarker = $derived(
    selected ? 'selection-selected' :
    focused ? 'selection-focused' :
    ''
  )

  // OUTER layer: focus/selection border (blue/yellow/transparent)
  // Uses border-transparent when inactive to keep layout stable (no shift)
  const outerBorderClass = $derived(
    selected ? 'border-2 border-yellow-500' :
    focused ? 'border-2 border-blue-500' :
    'border-2 border-transparent'
  )

  // INNER layer: decision border (green/red/gray)
  const innerBorderClass = $derived(
...
<!-- OUTER: focus/selection layer — always inside the component's border-box -->
<div class="relative flex flex-col {layoutSizing} {isRounded ? 'rounded-lg' : ''} {selectionMarker} {outerBorderClass}" data-testid="photo-frame">
  <!-- INNER: decision layer + content -->
  <div class="flex-1 min-h-0 flex flex-col {isRounded ? 'rounded-md' : ''} {innerBorderClass} bg-gray-900">
    <!-- Photo area -->
