...
- As a user, I want to merge two or more stacks into one new stack from the Stack Overview screen, because burst-gap auto-stacking is not always accurate and I need manual control over what constitutes a stack. **[P0] [DONE]**
...
- As a user, I want the last stack merge to be undoable (single-level undo). **[P1] [DONE]**
- As a user, I want all structural changes (merges, splits, restacks) to be recorded in a transaction log with timestamp and before/after state, similar to a git log, so I can review the history of how my stacks evolved. **[P0] [DONE]**
...
- As a user, I want decisions (keep / eliminate) to apply to the entire pair — one decision covers both files. **[P0] [DONE]**
...
- As a user, I want to see camera parameters (aperture, shutter speed, ISO, focal length, exposure compensation) displayed alongside each photo in Single View and comparison views, so I can learn what settings produce better results. **[P0] [DONE]**
...
- As a user, I want to promote all survivors of a stack's current round to the GemStack with a single key press (G), so I can over-promote liberally and cut later. **[P0] [NOT STARTED]**
- As a user, I want the GemStack to behave identically to a regular stack — same round engine, same Y/X decisions, same comparison view, same keyboard shortcuts. **[P0] [NOT STARTED]**
- As a user, I want the GemStack to be clearly marked as a special stack in StackOverview (distinct badge or label) so I can navigate to it quickly. **[P0] [NOT STARTED]**
- As a user, I want to be able to promote from a stack multiple times — if I re-enter a stack later and change my mind, pressing G again re-promotes the current survivors (replacing previous promotions from that stack). **[P1] [NOT STARTED]**
- As a user, I want to see which stacks have already promoted to GemStack (a promoted badge on the stack card) so I know which stacks still need attention. **[P1] [NOT STARTED]**
- As a user, I want the GemStack to show provenance — each photo in the GemStack displays which source stack it came from, so I maintain traceability. **[P1] [NOT STARTED]**
- As a user, I want to run rounds on the GemStack with no fixed target count — I cut until I'm satisfied, and the survivors are my final gems. **[P0] [NOT STARTED]**
- As a user, I want export to operate on the final survivors of the GemStack (the last committed round), treating them as the gem set. **[P0] [NOT STARTED]**
...
- As a user, I want three modes: Stack Overview (grid), Stack Focus (multi-photo grid), Single View (full-screen). **[P0] [DONE]**
- As a user, I want seamless switching between regular stacks and the GemStack without changing keyboard behavior. **[P0] [NOT STARTED]**
...
- As a user, I want simple filters in Stack Overview mode (date range, stack size, has-promoted / not-promoted, contains-RAW, contains-JPEG). **[P1] [NOT STARTED]**
...
- As a user, I want to export all GemStack survivors (from the last committed round) to a new folder (copy or hard-link). **[P0] [NOT STARTED]**
...
- As a user, I want to export a simple text/JSON list of GemStack survivor file paths (including both files of pairs). **[P1] [NOT STARTED]**
...
- As a user, I want to create custom text tags (e.g., "print", "family album", "portfolio") for organizing GemStack survivors beyond keep/eliminate. **[P1] [NOT STARTED]**
- As a user, I want to assign one or more tags to a logical photo after culling (tags are a secondary labeling step applied to GemStack survivors, not a replacement for keep/eliminate). **[P1] [NOT STARTED]**
...
- As a user, I want to filter GemStack photos by tag in export and GemStack views (e.g., export only "portfolio" tagged photos, or view only "print" candidates). **[P1] [NOT STARTED]**
